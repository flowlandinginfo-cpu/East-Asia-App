...
Load fonts via Google Fonts. Use CSS custom properties, `rem` for type, `px` for spacing/borders. Dark/light via `prefers-color-scheme` or class toggle.
...
Use CSS variables in `tailwind.config.js`:
```javascript
module.exports = {
  theme: {
    extend: {
...
Use `get_font_family_info` to verify fonts before writing styles. Direct hex values (no CSS variables). Dark mode as default canvas.
