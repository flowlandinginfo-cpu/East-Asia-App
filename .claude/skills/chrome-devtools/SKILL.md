...
3. **Trigger Action**: Use `trigger_extension_action` to open the popup or side panel.
4. **Verify Service Worker**: Use `evaluate_script` with `serviceWorkerId` to check extension state.
5. **Verify Page Behavior**: Navigate to a page and use `take_snapshot` to check content scripts.
